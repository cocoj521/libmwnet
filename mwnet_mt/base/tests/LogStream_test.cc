...
using mwnet_mt::string;

BOOST_AUTO_TEST_CASE(testLogStreamBooleans)
{
  mwnet_mt::LogStream os;
  const mwnet_mt::LogStream::Buffer& buf = os.buffer();
...
BOOST_AUTO_TEST_CASE(testLogStreamFmts)
{
  ...
  os << mwnet_mt::Fmt("%4d", 1);
  BOOST_CHECK_EQUAL(buf.toString(), string("   1"));
  os.resetBuffer();

  os << mwnet_mt::Fmt("%4.2f", 1.2);
  BOOST_CHECK_EQUAL(buf.toString(), string("1.20"));
  os.resetBuffer();

  os << mwnet_mt::Fmt("%4.2f", 1.2) << mwnet_mt::Fmt("%4d", 43);
