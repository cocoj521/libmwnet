...
bool CMsgFilesImpl::InitParams(const std::string& szPtCode, int ptid, int gateNo)
{
...
void CMsgFilesImpl::SetParams(int seconds, int maxFileSize)
{
...
int CMsgFilesImpl::InitMsgFiles(const std::string& szPtCode,
        int ptCode,
        int gateNo,
...
bool CMsgFilesImpl::bExistFile(const std::string& fileType)
{
...
int CMsgFilesImpl::WriteFile(const std::string& fileType, const std::string& data)
{
	// FIXME: 实现写文件操作
	std::lock_guard<std::mutex> lock(mutex_);
	...
	if (it != details_.end())
	{
		if (it->second)
		{
			return it->second->WriteToFile(data.c_str(), data.length());
		}
	}
...
void CMsgFilesImpl::SwitchMsgFiles()
{
	std::lock_guard<std::mutex> lock(mutex_);
	for (auto it = details_.begin(); it != details_.end(); ++it)
	{
...
int CMsgFilesImpl::GetLongTimeNoUploadFilesCnt(int min)
{
	int count = 0;
	// 扫描当天的
...
void CMsgFilesImpl::ScanfAndRenameTmpFile()
{
...
void CMsgFilesImpl::EnumTmpFile(const char* szFolderName, std::vector<std::string>& vFileName)
{
...
void CMsgFilesImpl::EnumNoUploadFiles(const char* szFolderName, int nMin, int& nCount)
{
	struct dirent* ent = NULL;
	DIR* pDir = opendir(szFolderName);
	if (pDir)
...
int CMsgFilesImpl::EnumNoUploadFiles(int nDaySpan, int nMin)
{
	int count = 0;
