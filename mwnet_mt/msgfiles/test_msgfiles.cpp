...
void send_tcp_rsp(uint64_t conn_uuid, const boost::any& conn, const boost::any& sessioninfo, const std::string& msg)
{
	printf("%ld\n", boost::any_cast<uint64_t>(sessioninfo));
	boost::any params;
	m_pNetServer1->SendMsgWithTcpServer(conn_uuid, conn, params, msg.c_str(), msg.size());
}
...
int func_on_readmsg_tcp(void* pInvoker, uint64_t conn_uuid, const boost::any& conn, const boost::any& sessioninfo, const char* szMsg, int nMsgLen, int& nReadedLen)
{
	...
	else 
	{
		...
		boost::any params;
		nReadedLen = nMsgLen;
		m_pNetServer->SendMsgWithTcpServer(conn_uuid, conn, params, szMsg, nMsgLen);
		*/
		nReadedLen = nMsgLen;
...
void* StartTcpServer(void* p)
{
	...
	if (pConfig)
	{
		...
		m_pNetServer1->StartTcpServer(pConfig->pInvoker, pConfig->listenport, 
			...
			pConfig->pOnConnection, pConfig->pOnReadMsg_tcp, pConfig->pOnSendOk, pConfig->pOnHighWaterMark,
	...
	char any[16] = {0};
{
	...
	*/
	...
	{
		sleep(1);
		printf("tcp  --- totallink:%d-totalreq:%ld\nhttp --- totallink:%d-totalreq:%ld\n\n", 
