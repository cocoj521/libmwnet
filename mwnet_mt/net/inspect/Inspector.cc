...
#include <mwnet_mt/net/inspect/ProcessInspector.h>
#include <mwnet_mt/net/inspect/PerformanceInspector.h>
#include <mwnet_mt/net/inspect/SystemInspector.h>
...
Inspector::Inspector()
    : loop_(nullptr),
      loopThread_(new EventLoopThread(nullptr, "inspect_thread")),
      server_(),
      processInspector_(),
      performanceInspector_(),
      systemInspector_()
{
}

void Inspector::init(const InetAddress& httpAddr,
                     const string& name)
{
  loop_ = loopThread_->startLoop();
  server_.reset(new HttpServer(loop_, httpAddr, "Inspector:"+name));
...
#endif
  loop_->runAfter(0, std::bind(&Inspector::start, this)); // little race condition
}
