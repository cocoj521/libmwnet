 ...
 * 它的超时回调函数，delete对象。
 */
class CurlRequest : public std::enable_shared_from_this<CurlRequest>,noncopyable
{
public:
	/**
	 ...
	 * @param req_uuid  [唯一标识]
	 */
	CurlRequest(const    std::string& url, uint64_t req_uuid, bool bKeepAlive, int req_type);

	~CurlRequest();

	// 初始化请求参数
	void initCurlRequest(const    std::string& url, uint64_t req_uuid, bool bKeepAlive, int req_type);
