...
void StringUtil::StringReplace(const std::string& subStr1,
...
void StrToHex(const char* src, size_t srcLen, std::string& dest)
{
	std::string tmp(2 * srcLen, '\0');

	static uint8_t h[] = "0123456789abcdef";

	for(size_t i = 0; i < srcLen; i++)
	{
		tmp[i] = h[static_cast<uint8_t>(src[i]) >> 4];
		tmp[i+1] = h[static_cast<uint8_t>(src[i+1]) & 0xf];				  
	}

	dest.swap(tmp);
}

void StringUtil::ToHexString(const std::string& src, std::string& dest)
{
	StrToHex(src.data(), src.size(), dest); 	
}

const char* StringUtil::ToHexString(const std::string& src)
{
	std::string dest;
	StrToHex(src.data(), src.size(), dest);
	return dest.c_str();
...
const char* StringUtil::ToHexString(const char* src, size_t len)
{
	std::string dest;
	StrToHex(src, len, dest);
	return dest.c_str();
}
